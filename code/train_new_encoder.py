...
#
#         # 每个人的 label
#         labels = np.full((num_channels,), label1)  # 每个数据点的标签是当前人的索引
#         all_label1.append(labels)
#
#         # 每个人每个通道的label
#         labels2 = np.arange(1, num_channels + 1)  # 每个样本的每个通道的标签是通道索引
...
criterion = nn.MSELoss()  # 使用均方误差作为重构误差的损失函数
...
best_encoded_output = None  # 用于存储最佳模型的编码器输出
